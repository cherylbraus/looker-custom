    ...
    try {
      ...
      var group_by_week = function group_by_week(arr) {
        return Object.values(data_ready.reduce(function (a, _ref2) {
          var weekStart = _ref2.week,
              ...
              budgetVal = _ref2.budget,
              forecastVal = _ref2.forecast,
              budgetTDVal = _ref2.budgetTD,
              forecastTDVal = _ref2.forecastTD,
              actualVal = _ref2.actual,
              actualContractVal = _ref2.actualContract,
              actualSpotVal = _ref2.actualSpot;
          var key = weekStart;
      ...
      var mousemove = function mousemove() {
        ...
        if (on_event.pageY < boundedHeight * .7) {
          ...
          tooltip.style("top", on_event.pageY - 160 + "px");
        }
      ...
      var weekAccessor = function weekAccessor(d) {
        return d.week;
      };
      ...
      var budgetTDAccessor = function budgetTDAccessor(d) {
        return d.budgetTD;
      };

      var forecastTDAccessor = function forecastTDAccessor(d) {
        return d.forecastTD;
      };
      ...
      var uniqueWeeks = _toConsumableArray(new Set(data_ready.map(function (obj) {
        return +obj.week;
      }))).sort(); // .map(tv => new Date(tv)).sort()


      var lastWeek = data_ready.filter(function (element) {
        return element.day.getTime() === new Date(new Date().setHours(0, 0, 0, 0)).getTime();
      })[0]['week'];
      var indexFourWkAgo = uniqueWeeks.indexOf(+lastWeek) - 3;
      var firstWeek = new Date(uniqueWeeks[indexFourWkAgo]); // const firstWeek = data_ready.filter(element => element.day.getTime() === new Date(new Date(new Date().getFullYear(), new Date().getMonth(), 1).setHours(0,0,0,0)).getTime())[0]['week']
      ...
      var yScale = linear_linear().range([boundedHeight, 0]);

      if (config.budgetlines == "true") {
        yScale.domain([0, Math.max(src_max(data_group, function (d) {
      ...
      var xDailyAxisGenerator = axisBottom().scale(xDailyScale).tickPadding(5).tickSizeOuter(0).tickSizeInner(0).tickFormat(function (d, i) {
        return d.slice(0, 1);
      }); // peripherals (axes)
      ...
      var yAxisGenerator = axisLeft().scale(yScale).tickPadding(10).ticks(9);

      if (config.yticklabels_show === "true") {
        yAxisGenerator.tickFormat(defaultLocale_format(config.ytick_format));
      } else {
      ...
      }).on("mouseleave", mouseleave); // draw weekly forecast lines

      var compBars = weekGroups.append("rect").attr("x", function (d) {
        return xScale(weekAccessor(d)) - 0.045 * xScale.bandwidth();
      }).attr("y", function (d) {
        return yScale(forecastTDAccessor(d));
      }).attr("width", xScale.bandwidth() + 0.09 * xScale.bandwidth()).attr("height", 3).attr("fill", "lightgrey").attr("stroke", "black").attr("stroke-width", 1).on("mouseover", mouseover).on("mousemove", function (d) {
        return mousemove({
          data: d,
          label: "forecastrect"
        });
      }).on("mouseleave", mouseleave); // draw monthly budget lines

      if (config.budgetlines === "true") {
        var budgetGoalLine = weekGroups.append("rect").attr("x", function (d) {
          return xScale(weekAccessor(d)) - 0.045 * xScale.bandwidth();
        }).attr("y", function (d) {
          return yScale(budgetTDAccessor(d));
      ...
      src_selectAll(".singleweek").each(function (e, i) {
        ...
        }).attr("y2", yScale(0)).attr("stroke", "black").attr("stroke-width", 2.5).attr("class", "daily-lines").on("mouseover", mouseover).on("mousemove", function (d) {
          return mousemove({
            data: d,
            label: "daycircle"
          });
      ...
      if (config.budgetlines == "true") {
        var legendBudget = legendContainer.append('g').classed("legend", true).attr("transform", "translate(110, -25)");
        legendBudget.append("text").attr("x", 20).attr("y", 0).style("text-anchor", "start").style("dominant-baseline", "middle").style("font-size", 11).text("WTD Budget");
        legendBudget.append("rect").attr("x", 0).attr("y", -2).attr("width", 15).attr("height", 3).attr("fill", "black");
      }

      if (config.dailylines == "true") {
        var legendDayGoal = legendContainer.append('g').classed("legend", true).attr("transform", "translate(".concat(legendXcoord, ", -25)"));
        legendDayGoal.append("text").attr("x", 20).attr("y", 0).style("text-anchor", "start").style("dominant-baseline", "middle").style("font-size", 11).text('Daily Budget');
        legendDayGoal.append("line").attr("x1", 0).attr("x2", 15).attr("y1", -1).attr("y2", -1).attr("stroke", "grey").attr("stroke-width", 2).attr("stroke-dasharray", "5,3");
      }
    } catch (error) {
      if (environment == "prod") {
