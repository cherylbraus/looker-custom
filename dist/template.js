...
// CONCATENATED MODULE: ./src/integration/template/linedropdown.js
...
var linedropdown_object = {
  ...
  updateAsync: function updateAsync(data, element, config, queryResponse, details, done) {
    ...
    try {
      var group_by_month = function group_by_month(arr) {
        return Object.values(arr.reduce(function (a, _ref) {
      ...
      var redraw = function redraw() {
        var compperiod = jquery("#vis-options option:selected").val();
        console.log("COMP PERIOD", compperiod);
        var svg = src_select(element).select("svg").html("").attr("width", "100%").attr("height", "100%");
        var group = svg.append("g").attr("transform", "translate(".concat(margin.left, ", ").concat(margin.top, ")")).attr("width", "100%").attr("height", boundedHeight + "px").classed("group", true);
        var data_line = data_ready.filter(function (d) {
        ...
        var data_group = group_by_month(data_line);
        console.log("data_group", data_group); // calculate cumulative actuals

        var cumHighs = [];
        data_group.forEach(function (m, i) {
          var c = m.days.map(function (d, j) {
            return {
              day: d.day,
              actcum: m.days.slice(0, j + 1).map(function (_ref2) {
          ...
          console.log("c - ".concat(i), c);
          m.days = m.days.map(function (item, j) {
            return Object.assign({}, item, c[j]);
          });
          cumHighs.push(src_max(m.days, function (d) {
            return d.actcum;
          }));
        });
        console.log("data_group cum", data_group);
        console.log("cumHighs", cumHighs); // X-Axis

        var xScale = linear_linear().domain(src_extent(data_line, function (d) {
          return daynumAccessor(d);
        })).range([0, boundedWidth]); // Y-Axis

        var yScale = linear_linear().domain([0, Math.max.apply(Math, cumHighs)]).range([boundedHeight, 0]); // Draw Data

        var line = src_line().defined(function (d) {
          return d.actcum != null && d.actnum != 0;
        }).curve(natural).x(function (d) {
          return xScale(d.daynum);
        }).y(function (d) {
          return yScale(d.actcum);
        });
        console.log("made line", line);
        var currentLine = group.append("path").data([data_group[1].days]).attr("class", "currentmonth").attr("d", line).attr("fill", "none").attr("stroke", "#323232").attr("stroke-width", "2.5px");
        var priorLine = group.append("path").data([data_group[0].days]).attr("class", "priormonth").attr("d", line).attr("fill", "none").attr("stroke", "#323232").attr("stroke-width", "2px").attr("stroke-dasharray", "5 3"); // calculation of MTD comparison as % change

        var current_total = data_group[1].days[data_group[1].days.length - 1].actcum;

        var prior_total = function () {
          if (data_group[1].days.length > data_group[0].days.length) {
            // if more days in current month elapsed than in prior had
        ...
        var change = formatter.format((current_total - prior_total) / prior_total);
        console.log("change", change);
        var changeContainer = group.append('g').attr("transform", "translate(70, 30)").classed("changeContainer", true);
        var changeNumber = changeContainer.append("g").classed("change", true).attr("transform", "translate(0, 0)");
        changeNumber.append("text").attr("x", 10).attr("y", 0).style("text-anchor", "middle").style("dominant-baseline", "middle").style("font-size", 28).attr("fill", function () {
        ...
        var changeText = changeContainer.append("g").classed("change", true).attr("transform", "translate(0, 25)");
        changeText.append("text").attr("x", 10).attr("y", 0).style("text-anchor", "middle").style("dominant-baseline", "middle").style("font-size", 12).attr("fill", function () {
          if (+current_total >= +prior_total) {
            return "#0072b5";
          } else {
            return "#D76106";
          ...
          var temp = new Date(data_group[0].month).toDateString();
          return "MTD Change vs ".concat(temp.split(" ")[1], "-").concat(temp.split(" ")[3]);
        }); // legend

        if (config.show_legend == "true") {
          var legendContainer = group.append("g").attr("transform", "translate(".concat(boundedWidth - 120, ",").concat(boundedHeight - 40, ")")).classed("legendContainer", true);
          var legendCurrent = legendContainer.append("g").classed("legend", true).attr("transform", "translate(6, 2)");
          legendCurrent.append("line").attr("x1", 6).attr("x2", 25).attr("y1", 2).attr("y2", 2).attr("stroke", "#323232").attr("stroke-width", "2.5px");
          legendCurrent.append("text").attr("x", 30).attr("y", 2).style("text-anchor", "start").style("dominant-baseline", "middle").style("font-size", 11).attr("color", "#323232").text("Current Month");
          var legendPrior = legendContainer.append("g").classed("legend", true).attr("transform", "translate(6, 10)");
          legendPrior.append("line").attr("x1", 6).attr("x2", 25).attr("y1", 10).attr("y2", 10).attr("stroke", "#323232").attr("stroke-width", "2.5px").attr("stroke-dasharray", "5 3");
          legendPrior.append("text").attr("x", 30).attr("y", 10).style("text-anchor", "start").style("dominant-baseline", "middle").style("font-size", 11).attr("color", "#323232").text(function () {
            var temp = new Date(data_group[0].month).toDateString();
            return "".concat(temp.split(" ")[1], "-").concat(temp.split(" ")[3]);
          });
        }
      };
      ...
      var boundedHeight = height - margin.top - margin.bottom; // TOOLTIPS ---------------------------------------------------------------
      // END TOOLTIPS -----------------------------------------------------------
      // LOAD DATA -----------------------------------------------------------
      ...
      var dayAccessor = function dayAccessor(d) {
        return d.day;
      };

      var daynumAccessor = function daynumAccessor(d) {
        return d.daynum;
      };
      ...
      var uniqueMonths = _toConsumableArray(new Set(data_ready.map(function (obj) {
        return +obj.month;
      }))).sort().reverse();

      console.log("uniqueMonths", uniqueMonths);
      var currentMonth = new Date(uniqueMonths[0]);
      var priorMonth = new Date(uniqueMonths[1]);
      var priorYear = new Date(currentMonth.getFullYear() - 1, currentMonth.getMonth()); // getMonth and new Date() are 0 index

      var prior2Year = new Date(currentMonth.getFullYear() - 2, currentMonth.getMonth());
      var prior3Year = new Date(currentMonth.getFullYear() - 3, currentMonth.getMonth());
      console.log("different dates", currentMonth, priorMonth, priorYear, prior2Year, prior3Year);
      var dateskeep = [currentMonth.toDateString(), priorMonth.toDateString(), priorYear.toDateString(), prior2Year.toDateString(), prior3Year.toDateString()];
      data_ready = data_ready.filter(function (d) {
        return dateskeep.includes(d.month.toDateString());
      });
      console.log("big filter", data_ready);

      var filteredUniqueMonths = _toConsumableArray(new Set(data_ready.map(function (obj) {
        return obj.month.toDateString();
      })));

      console.log("filteredUniqueMonths", filteredUniqueMonths);
      var indexofcurrent = filteredUniqueMonths.indexOf(currentMonth.toDateString());
      filteredUniqueMonths.splice(indexofcurrent, 1);
      console.log("filteredUniqueMonths", filteredUniqueMonths);
      var dateMap = [];
      filteredUniqueMonths.forEach(function (d, i) {
        var entry = {};
        entry['dropdownval'] = d;
        entry['label'] = "".concat(d.split(" ")[1], "-").concat(d.split(" ")[3]);
      ...
      listDropdown.empty();
      dateMap.forEach(function (entry, i) {
        if (i == 0) {
          listDropdown.append(jquery("<option></option>").attr("value", entry['dateval']).text(entry['label']).attr("selected", "selected"));
        } else {
          listDropdown.append(jquery("<option></option>").attr("value", entry['dateval']).text(entry['label']));
        }
      });
...
var template_keys = Object.keys(linedropdown_object.options);
console.log("object.options", linedropdown_object.options);
console.log("Object", Object.keys(linedropdown_object.options));
console.log("keys", template_keys);
template_keys.forEach(function (entry, i) {
  var array_name = linedropdown_object.options[entry].label;
  ...
  if (["radio", "select"].includes(linedropdown_object.options[entry].display)) {
    var array_values = linedropdown_object.options[entry].values;
    array_values.forEach(function (ent) {
      var str;

      if (ent[Object.keys(ent)[0]] == linedropdown_object.options[entry]["default"]) {
  ...
  } else if (linedropdown_object.options[entry].display == "number") {
    var str;
    str = "<input type='number' internal_cat='" + template_keys[i] + "' internal_value='" + linedropdown_object.options[entry]["default"] + "' id='" + template_keys[i] + "' min='0' name='" + linedropdown_object.options[entry]["label"] + "' value='" + linedropdown_object.options[entry]["default"] + "'></input><label class='form-label' for='" + linedropdown_object.options[entry]["label"] + "'>" + linedropdown_object.options[entry]["label"] + "</label>";
    form.append(str);
  } else if (linedropdown_object.options[entry].display == "text") {
    var _str;

    _str = "<input type='text' internal_cat='" + template_keys[i] + "' internal_value='" + linedropdown_object.options[entry]["default"] + "' id='" + template_keys[i] + "' name='" + linedropdown_object.options[entry]["label"] + "' value='" + linedropdown_object.options[entry]["default"] + "'></input><label class='form-label' for='" + linedropdown_object.options[entry]["label"] + "'>" + linedropdown_object.options[entry]["label"] + "</label>";
    form.append(_str);
  } else if (linedropdown_object.options[entry].type == "boolean") {
    var _array_values = ["true", "false"];

    _array_values.forEach(function (ent) {
      var str;

      if (ent == linedropdown_object.options[entry]["default"]) {
  ...
  } else if (linedropdown_object.options[entry].type == "array") {
    var _array_values2 = linedropdown_object.options[entry]["default"]; // console.log("array_values", array_values)
...
json("http://localhost:3001/dataLineDropdown").then(function (data) {
  var todays_options = {};
  ...
  linedropdown_object.updateAsync(data.data, src_select("#vis")._groups[0][0], todays_options, data.queryResponse, details, template_done, this_environment); // On change to options, loop through selections and then redraw chart

  jquery('input:radio').on("click", function () {
    jquery('input:radio:checked').each(function () {
      todays_options[this.attributes.internal_cat.value] = this.attributes.internal_value.value;
    });
    linedropdown_object.updateAsync(data.data, src_select("#vis")._groups[0][0], todays_options, data.queryResponse, details, template_done, this_environment);
  });
