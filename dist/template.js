...
// CONCATENATED MODULE: ./src/integration/template/conwinrate.js
  ...
  updateAsync: function updateAsync(data, element, config, queryResponse, details, done) {
    ...
    try {
      ...
      var redraw = function redraw() {
        ...
        var data_lane = data_ready.filter(function (d) {
          return +d.lane === +lan;
        });
        ...
        var ratio_keys = data_lane[0]["winrates"].map(function (d) {
          return +d.ratio;
        });
        var ratio_min = Math.min.apply(Math, _toConsumableArray(ratio_keys));
        var ratio_max = Math.max.apply(Math, _toConsumableArray(ratio_keys));
        console.log("ratio keys", ratio_keys);
        console.log("ratio min/max", ratio_min, ratio_max);
        var rates = [];
        data_lane.forEach(function (d, i) {
          d.winrates.forEach(function (dd, ii) {
            rates.push(+dd.value);
          });
        });
        var rate_min = Math.min.apply(Math, rates);
        var rate_max = Math.max.apply(Math, rates);
        console.log("rate min/max", rate_min, rate_max); // AXES -----------------------------------------------------------

        var xScale = linear_linear().domain([ratio_min, ratio_max]).range([0, boundedWidth]);
        ...
        var line = src_line().defined(function (d) {
          return d.ratio != null && d["value"] != null;
      ...
      console.log("heights", height, boundedHeight); // LOAD DATA -----------------------------------------------------------
      ...
      var laneAccessor = function laneAccessor(d) {
        return d.lane;
      };

      var modelAccessor = function modelAccessor(d) {
        return d.model;
      };

      var winrateAccessor = function winrateAccessor(d) {
        return d.winrate;
      };
      ...
      data_ready.forEach(function (d, i) {
        d.winrates = [];
        Object.keys(d.winrate).forEach(function (dd, ii) {
          var newObj = {
            ratio: +dd,
      ...
      console.log("data_ready2", data_ready); // DROPDOWN -----------------------------------------------------------
      ...
      lanes.forEach(function (d, i) {
        if (i == 0) {
          ...
          listDropdown.append(jquery("<option></option>").attr("value", d).text(d));
        }
      });
...
var template_keys = Object.keys(conwinrate_object.options);
console.log("object.options", conwinrate_object.options);
console.log("Object", Object.keys(conwinrate_object.options));
console.log("keys", template_keys);
template_keys.forEach(function (entry, i) {
  var array_name = conwinrate_object.options[entry].label;
  ...
  if (["radio", "select"].includes(conwinrate_object.options[entry].display)) {
    var array_values = conwinrate_object.options[entry].values;
    array_values.forEach(function (ent) {
      var str;

      if (ent[Object.keys(ent)[0]] == conwinrate_object.options[entry]["default"]) {
  ...
  } else if (conwinrate_object.options[entry].display == "number") {
    var str;
    str = "<input type='number' internal_cat='" + template_keys[i] + "' internal_value='" + conwinrate_object.options[entry]["default"] + "' id='" + template_keys[i] + "' min='0' name='" + conwinrate_object.options[entry]["label"] + "' value='" + conwinrate_object.options[entry]["default"] + "'></input><label class='form-label' for='" + conwinrate_object.options[entry]["label"] + "'>" + conwinrate_object.options[entry]["label"] + "</label>";
    form.append(str);
  } else if (conwinrate_object.options[entry].display == "text") {
    var _str;

    _str = "<input type='text' internal_cat='" + template_keys[i] + "' internal_value='" + conwinrate_object.options[entry]["default"] + "' id='" + template_keys[i] + "' name='" + conwinrate_object.options[entry]["label"] + "' value='" + conwinrate_object.options[entry]["default"] + "'></input><label class='form-label' for='" + conwinrate_object.options[entry]["label"] + "'>" + conwinrate_object.options[entry]["label"] + "</label>";
    form.append(_str);
  } else if (conwinrate_object.options[entry].type == "boolean") {
    var _array_values = ["true", "false"];

    _array_values.forEach(function (ent) {
      var str;

      if (ent == conwinrate_object.options[entry]["default"]) {
  ...
  } else if (conwinrate_object.options[entry].type == "array") {
    var _array_values2 = conwinrate_object.options[entry]["default"]; // console.log("array_values", array_values)
...
json("http://localhost:3001/dataConWinRate3").then(function (data) {
  ...
  conwinrate_object.updateAsync(data.data, src_select("#vis")._groups[0][0], todays_options, data.queryResponse, details, template_done, this_environment); // On change to options, loop through selections and then redraw chart

  jquery('input:radio').on("click", function () {
    jquery('input:radio:checked').each(function () {
      todays_options[this.attributes.internal_cat.value] = this.attributes.internal_value.value;
    });
    conwinrate_object.updateAsync(data.data, src_select("#vis")._groups[0][0], todays_options, data.queryResponse, details, template_done, this_environment);
  });
