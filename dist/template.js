    ...
    if (typeof config.where_values == 'undefined' || typeof config.where_chart == 'undefined') {
      ...
      try {
        ...
        var createTable = function createTable(data_insert, element) {
          ...
          $('#vis').contents(':not(style)').remove();
          var headings_dim = data_insert; // console.log("headings_dim", headings_dim)
          ...
          $.each(headings_dim, function () {
            ...
            console.log("lookup_this", this); // this is the current heading
          ...
          html += '</tr></thead>'; // build table body

          html += '<tbody>'; // console.log("createTable 1")
          ...
          var _loop = function _loop(i, len) {
            // console.log("i 594", i)
            html += '<tr><td class="table-index">' + (i + 1) + '</td>'; // build each cell using the heading's ith element
            ...
            $.each(headings_dim, function () {
              ...
              if (this == "chart_column") {
                ...
                var chart_id = "chart_column-" + i;
                html += '<td class="no-padding"><div class="chart_column" id=' + chart_id + '></div></td>';
              } else if (this == "second_chart_column") {
                // console.log("SECOND CHART COLUN - 5")
                var _chart_id = "second_chart_column-" + i;

                html += '<td class="no-padding"><div class="second_chart_column" id=' + _chart_id + '></div></td>';
              } else if (this == "last_week_column") {
                // console.log("LAST WEEK COLUMN - 6")
                // Add section for triangle up/down trends
                var last_week_id = "last_week_column-" + i;
                var last_week_class;
                var direction_icon;
                var date_keys = Object.keys(data[i][measures[where_values[0]].name]); // console.log("date_keys pre", date_keys)

                date_keys = date_keys.filter(function (entry) {
                  return entry[0] != "$";
                }); // console.log("date_keys", date_keys)

                var last_week_data; // console.log("test", data[i][measures[where_values[0]].name])
                ...
                var _last_week_id = "second_last_week_column-" + i;

                var _last_week_class;

                var _direction_icon;

                var _date_keys = Object.keys(data[i][measures[where_values[1]].name]);

                _date_keys = _date_keys.filter(function (entry) {
                  return entry[0] != "$";
                });

                var _last_week_data;

                if (data[i][measures[where_values[1]].name][_date_keys[_date_keys.length - 1]]["value"] != null) {
                  if (data[i][measures[where_values[1]].name][_date_keys[_date_keys.length - 1]]["rendered"]) {
                    _last_week_data = data[i][measures[where_values[1]].name][_date_keys[_date_keys.length - 1]]["rendered"];
                  } else {
                    _last_week_data = data[i][measures[where_values[1]].name][_date_keys[_date_keys.length - 1]]["value"];
                  }
                } else {
                  _last_week_data = "<span class='null-val'>∅</span>";
                }

                if (data[i][measures[where_values[1]].name][_date_keys[_date_keys.length - 1]]["value"] != null && data[i][measures[where_values[1]].name][_date_keys[_date_keys.length - 2]]["value"] != null && data[i][measures[where_values[1]].name][_date_keys[_date_keys.length - 1]]["value"] < data[i][measures[where_values[1]].name][_date_keys[_date_keys.length - 2]]["value"]) {
                  _last_week_class = "value-down";
                  _direction_icon = '<div id="second-chart-triangle-' + i + '"></div>';
                } else if (data[i][measures[where_values[1]].name][_date_keys[_date_keys.length - 1]]["value"] != null && data[i][measures[where_values[1]].name][_date_keys[_date_keys.length - 2]]["value"] != null && data[i][measures[where_values[1]].name][_date_keys[_date_keys.length - 1]]["value"] > data[i][measures[where_values[1]].name][_date_keys[_date_keys.length - 2]]["value"]) {
                  _last_week_class = "value-up";
                  _direction_icon = '<div id="second-chart-triangle-' + i + '"></div>';
                } else {
                  _last_week_class = "value-even";
                  _direction_icon = '<div style="display:none" id="second-chart-triangle-' + i + '"></div>';
                }

                html += '<td class="triangle-flex"><div class=' + _last_week_class + ' id=' + _last_week_id + '><div class="change-figure">' + _last_week_data + '</div>' + _direction_icon + '</div></td>';
              } else if (this.split("-")[0] == "row_total_heading") {
                ...
                } else {// console.log("ROW TOTAL HEADING - 8d - NO VALUE/RENDERED")
        ...
        var drawOnTable = function drawOnTable(data_insert, element) {
          ...
          var height_first;
          var width_second;
          var height_second; // DETERMINE HEIGHT OF EACH SVG IS BASED ON FIRST ROW HEIGHT IF SINGLE Y-SCALE

          if (config.y_axis_scale_fixed == "true") {
            if (config.where_chart.length > 0) {
              var node_get_width = src_select("#chart_column-0");
              var new_node_width = node_get_width.select(function () {
                return this.parentNode;
              });
              width_first = new_node_width.node().getBoundingClientRect().width; // console.log("check width of 1st", node_get_width, new_node_width, width_first)
            }

            if (config.where_chart.length > 0) {
              var node_get_height = src_select("#chart_column-0");
              var new_node_height = node_get_height.select(function () {
                return this.parentNode;
              });
              height_first = new_node_height.node().getBoundingClientRect().height; // console.log("check height of 1st", node_get_height, new_node_height, height_first)
            }

            if (config.where_chart.length > 1) {
              var node_get_width_2 = src_select("#second_chart_column-0");
              var new_node_width_2 = node_get_width_2.select(function () {
                return this.parentNode;
              });
            ...
            if (config.where_chart.length > 1) {
              var node_get_height_2 = src_select("#second_chart_column-0");
              var new_node_height_2 = node_get_height_2.select(function () {
                return this.parentNode;
              });
          ...
          if (where_values.length < 1) {
            ...
            if (config.y_axis_scale_fixed == "true") {
              var _loop2 = function _loop2(j, len) {
                var dataset_one = [];
                var dataset_one_keys = Object.keys(data[j][measures[where_values[0]].name]); // console.log("dataset_one_keys", dataset_one_keys)

                dataset_one_keys.forEach(function (entry) {
                  if (entry[0] != "$") {
                    dataset_one.push({
                      date: entry,
                      value: data[j][measures[where_values[0]].name][entry]["value"]
                    });
                  }
                }); // console.log("dataset_one_keys done", dataset_one_keys)

                col_one_max = src_max([col_one_max, src_max(dataset_one, function (ent) {
                  return ent.value;
                })]);
                col_one_min = src_min([col_one_min, src_min(dataset_one, function (ent) {
                  return ent.value;
                })]);

                if (where_values.length > 1) {
                  var dataset_two = [];
                  var dataset_two_keys = Object.keys(data[j][measures[where_values[1]].name]);
                  dataset_two_keys.forEach(function (entry) {
                    if (entry[0] != "$") {
                      dataset_two.push({
                        date: entry,
                        value: data[j][measures[where_values[1]].name][entry]["value"]
                      });
                    }
                  });
                  col_two_max = src_max([col_two_max, src_max(dataset_two, function (ent) {
                    return ent.value;
                  })]);
                  col_two_min = src_min([col_two_min, src_min(dataset_two, function (ent) {
                    return ent.value;
                  })]);
                }
              };
            ...
            var _loop3 = function _loop3(i, _len) {
              var dataset_one = [];
              var dataset_one_keys = Object.keys(data[i][measures[where_values[0]].name]);
              dataset_one_keys.forEach(function (entry) {
                // entry is an individual column from list of date columns and total
                if (entry[0] != "$") {
                  dataset_one.push({
                    date: entry,
                    value: data[i][measures[where_values[0]].name][entry]["value"],
                    rendered: data[i][measures[where_values[0]].name][entry]["rendered"]
                  });
                }
              ...
              if (config.y_axis_scale_fixed == "false") {
                if (config.where_chart.length > 0) {
                  var _node_get_width = src_select("#chart_column-".concat(i));

                  var _new_node_width = _node_get_width.select(function () {
                    return this.parentNode;
                  });

                  width_first = _new_node_width.node().getBoundingClientRect().width; // console.log("check width of 1st", node_get_width, new_node_width, width_first)
                }

                if (config.where_chart.length > 0) {
                  var _node_get_height = src_select("#chart_column-".concat(i));

                  var _new_node_height = _node_get_height.select(function () {
                    return this.parentNode;
                  });

                  height_first = _new_node_height.node().getBoundingClientRect().height; // console.log("check height of 1st", node_get_height, new_node_height, height_first)
                }

                if (config.where_chart.length > 1) {
                  var _node_get_width_ = src_select("#second_chart_column-".concat(i));

                  var _new_node_width_ = _node_get_width_.select(function () {
                    return this.parentNode;
                  });

                  width_second = _new_node_width_.node().getBoundingClientRect().width;
                }

                if (config.where_chart.length > 1) {
                  var _node_get_height_ = src_select("#second_chart_column-".concat(i));

                  var _new_node_height_ = _node_get_height_.select(function () {
                    return this.parentNode;
                  });

                  height_second = _new_node_height_.node().getBoundingClientRect().height;
                }
              ...
              if (config.y_axis_scale_fixed == "false") {
                col_one_max = src_max(dataset_one, function (ent) {
                  return ent.value;
                });
                col_one_min = src_min(dataset_one, function (ent) {
                  return ent.value;
                });
              ...
              var chart_id_grabber = "#chart_column-" + i;
              var triangle_id_grabber = "#chart-triangle-" + i;
              var svg = src_select(chart_id_grabber).append('svg').html('').attr('width', width_first).attr('height', height_first).classed('first-svg', true);
              var group = svg.append('g').attr("transform", "translate(" + margin.left + "," + margin.top + ")").attr("width", width_first - margin.left - margin.right).attr("height", height_first - margin.top - margin.bottom).classed("group", true);
              icons = src_select(triangle_id_grabber).append("svg").attr("width", 30).attr("height", 30).append("g").attr("transform", "translate(15,18)");
              ...
              if (config.first_chart_type == "line") {
                group.append("path").data([dataset_one]).attr("class", "line").attr("d", line).attr("stroke", function (d) {
                  if (config.directionality == "true") {
                ...
                group.selectAll(".dot").data(dataset_one.filter(function (d, i) {
                  return d.value !== null;
                ...
                group.append("path").data([dataset_one]).attr("class", "area").attr("d", area).attr("stroke", function (d) {
                  if (config.directionality == "true") {
                    if (dataset_one[dataset_one.length - 1].value != null && dataset_one[0].value > dataset_one[dataset_one.length - 1].value) {
                      return "#D76106";
                    } else if (dataset_one[dataset_one.length - 1].value != null && dataset_one[0].value < dataset_one[dataset_one.length - 1].value) {
                ...
                var xBar = band().domain(dataset_one.map(function (d, i) {
                  return d.date;
                })).range([0, width_first - margin.left - margin.right]).padding(0.05);
                group.selectAll(".bar").data(dataset_one).enter().append("rect").attr("class", "bar").attr("x", function (d) {
                  return xBar(d.date);
                }).attr("y", function (d) {
              ...
              var arr_text = [];
              var val_extent = src_extent(dataset_one, function (d, i) {
                return d.value;
              });
              var maxValValues = {};
              var minValValues = {};
              ...
              if (isNaN(+val_extent[0])) {
                ...
                if (config.display_values == "true") {
                  filter = group.append("defs").append("filter").attr("id", "blur").append("feGaussianBlur").attr("stdDeviation", .8);
              ...
              if (where_values.length > 1) {
                var dataset_two = [];
                var dataset_two_keys = Object.keys(data[i][measures[where_values[1]].name]);
                dataset_two_keys.forEach(function (entry) {
                  if (entry[0] != "$") {
                    dataset_two.push({
                      date: entry,
                      value: data[i][measures[where_values[1]].name][entry]["value"],
                      rendered: data[i][measures[where_values[1]].name][entry]["rendered"]
                    });
                  }
                });
                ...
                var line2 = src_line().defined(function (d) {
                  return d.value != null;
                }).x(function (d) {
                  return x(parseTime(d.date));
                }).y(function (d) {
                ...
                var area2 = d3_shape_src_area().x(function (d) {
                  return x(parseTime(d.date));
                }).y0(function (d) {
                  return y2(0);
                }).y1(function (d) {
                ...
                var second_chart_id_grabber = "#second_chart_column-" + i;
                var second_triangle_id_grabber = "#second-chart-triangle-" + i;
                var svg_two = src_select(second_chart_id_grabber).append('svg').html('').attr('width', width_second).attr('height', height_second).classed('second-svg', true);
                var group_two = svg_two.append('g').attr("transform", "translate(" + margin.left + "," + margin.top + ")").attr("width", width_second - margin.left - margin.right).attr("height", height_second - margin.top - margin.bottom).classed("group", true);
                icons_two = src_select(second_triangle_id_grabber).append("svg").attr("width", 30).attr("height", 30).append("g").attr("transform", "translate(15,18)");
                var tri_two = icons_two.append('path').attr("d", src_symbol().type(triangle).size(30));

                if (config.directionality == "true") {
                  if (dataset_two[dataset_two.length - 1].value < dataset_two[dataset_two.length - 2].value) {
                    tri_two.attr("transform", "rotate(180)").style("fill", "#D76106");
                  } else if (dataset_two[dataset_two.length - 1].value > dataset_two[dataset_two.length - 2].value) {
                    tri_two.style("fill", "#0072b5");
                  }
                } else {
                  if (dataset_two[dataset_two.length - 1].value < dataset_two[dataset_two.length - 2].value) {
                ...
                if (config.second_chart_type == "line") {
                  group_two.append("path").data([dataset_two]).attr("class", "line").attr("d", line2).attr("stroke", function (d) {
                    if (config.directionality == "true") {
                  ...
                  group_two.selectAll(".dot-two").data(dataset_two.filter(function (d, i) {
                    return d.value !== null;
                  })).enter().append("circle").attr("class", "dot-two").attr("cx", line2.x()).attr("cy", line2.y()).attr("r", 1).attr("fill", function (d) {
                    if (config.directionality == "true") {
                      if (dataset_two[dataset_two.length - 1].value != null && dataset_two[0].value > dataset_two[dataset_two.length - 1].value) {
                        return "#D76106";
                      } else if (dataset_two[dataset_two.length - 1].value != null && dataset_two[0].value < dataset_two[dataset_two.length - 1].value) {
                  ...
                  group_two.append("path").data([dataset_two]).attr("class", "area").attr("d", area2).attr("stroke", function (d) {
                    if (config.directionality == "true") {
                      if (dataset_two[dataset_two.length - 1].value != null && dataset_two[0].value > dataset_two[dataset_two.length - 1].value) {
                        return "#D76106";
                      } else if (dataset_two[dataset_two.length - 1].value != null && dataset_two[0].value < dataset_two[dataset_two.length - 1].value) {
                  ...
                  var _xBar = band().domain(dataset_two.map(function (d, i) {
                    return d.date;
                  })).range([0, width_first - margin.left - margin.right]).padding(0.05);

                  group_two.selectAll(".bar").data(dataset_two).enter().append("rect").attr("class", "bar").attr("x", function (d) {
                    return _xBar(d.date);
                  }).attr("y", function (d) {
                    return y2(Math.max(0, d.value));
                  }).attr("width", _xBar.bandwidth()).attr("height", function (d) {
                ...
                var arr_text_two = [];
                var val_extent_two = src_extent(dataset_two, function (d, i) {
                  return d.value;
                });
                var maxValValues_two = {};
                var minValValues_two = {};
                ...
                if (isNaN(+val_extent_two[0])) {
                  ...
                  arr_text_two.push({
                    "value": val_extent_two[1],
                    "date": maxValValues_two["date"],
                    "rendered": maxValValues_two["rendered"]
                  });
                  arr_text_two.push({
                    "value": val_extent_two[0],
                    "date": minValValues_two["date"],
                    "rendered": minValValues_two["rendered"]
                  });

                  if (config.display_values == "true") {
                    filter_two = group_two.append("defs").append("filter").attr("id", "blur_two").append("feGaussianBlur").attr("stdDeviation", .8);
                    group_two.selectAll(".min-max-text-background_two").data(arr_text_two).enter().append("text").attr("class", "min-max-text-background_two").attr("x", function (d, i) {
                      if (pivots[0].time_interval.name == "month" && x(parseTime(d.date)) > 20) {
                        return x(parseTime(d.date));
                      } else {
                        return x(parseTime(d.date));
                    ...
                    }).attr("font-size", 8).attr("font-weight", 700).attr("fill", "white").attr("text-anchor", function (d, i) {
                      if (x(parseTime(d.date)) < 30) {
                        return "beginning";
                      } else {
                        return "end";
                      }
                    }).style("opacity", .4).attr("filter", "url(#blur_two)");
                    group_two.selectAll(".min-max-text_two").data(arr_text_two).enter().append("text").attr("class", "min-max-text_two").attr("x", function (d, i) {
                      if (pivots[0].time_interval.name == "month" && x(parseTime(d.date)) > 20) {
                        return x(parseTime(d.date));
                      } else {
                        return x(parseTime(d.date));
        ...
        var parseTimeDay = timeParse("%Y-%m-%d");
        var parseTimeMonth = timeParse("%Y-%m");
        var parseTimeYear = timeParse("%Y");
        var parseTimeSecond = timeParse("%Y-%m-%d %H:%M:%S");
        var parseTime;
        ...
        var row_totals = [];
        var column_headers = [];
        var final_measures = []; // Get the header names for the dimensions
        ...
        var y = linear_linear().range([46, 0]);
        var y2 = linear_linear().range([46, 0]);
...
json("http://localhost:3001/dataSparklineWorks").then(function (data) {
  //dataMCSmap2 dataSparklineWorks
  var todays_options = {};
