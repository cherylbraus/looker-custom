...
// CONCATENATED MODULE: ./src/integration/template/sparklinesNew.js
...
var sparklinesNew_object = {
  ...
  options: {
    ...
    add_last_period_stats: {
      // ignore_last_week = false
      section: "Setup",
      order: 9,
      type: "boolean",
  ...
  updateAsync: function updateAsync(data, element, config, queryResponse, details, done) {
    ...
    if (typeof config.where_values == 'undefined' || typeof config.where_chart == 'undefined') {
      ...
      var error = '<div class="error-container"><div class="error-header">Loading</div><div class="error">Wait until chart type is fully loaded.</div></div>';
      $('#vis').append(error);
    } else if (queryResponse.has_row_totals == false) {
      $('#vis').contents(':not(style)').remove();
      var _error = '<div class="error-container"><div class="error-header">Row totals required</div><div class="error">Sparklines requires row totals. Check row totals box and re-run.</div></div>';
      $('#vis').append(_error);
      ...
      $('#vis').contents(':not(style)').remove();
      var _error2 = '<div class="error-container"><div class="error-header">Options mismatch</div><div class="error">Number of measures charted and chart columns must be equal.</div></div>';
      $('#vis').append(_error2);
      ...
      $('#vis').contents(':not(style)').remove();
      var _error3 = '<div class="error-container"><div class="error-header">Index out of range</div><div class="error">That value is out of the range of possible measures.</div></div>';
      $('#vis').append(_error3);
    } else if (config.total_period_count > queryResponse.fields.measure_like.length - 1) {
      // FIX THIS
      $('#vis').contents(':not(style)').remove();
      var _error4 = '<div class="error-container"><div class="error-header">Value out of range</div><div class="error">Number of periods is larger than periods included in data.</div></div>';
      $('#vis').append(_error4);
    } else {
      try {
        // -----------------------------------------------------------------------------------------
        // Function that creates the table
        var createTable = function createTable(data_insert, element) {
          // Clear out previous charts/code, except keep style tag
          $('#vis').contents(':not(style)').remove();
          var headings_dim = data_insert;
          var html = '<table>';
          html += '<thead><tr><th class="table-index"></th>'; // cycle through each dimension heading $.each() iterates over array
          // function is passed array index & corresponding value referenced using "this"

          $.each(headings_dim, function () {
            var found = false;
            var lookup_this = this;
              ...
              if (lookup_this in OPTION_MAP) {
                ...
                if (OPTION_MAP[lookup_this]["type"] === "Trend" || config.add_last_period_stats == "true") {
                  if (OPTION_MAP[lookup_this]["option"].length == 0) {
                    html += "<th class=\"insert-column\">" + measures[OPTION_MAP[lookup_this]["where"]].label_short + " ".concat(OPTION_MAP[lookup_this]["type"], "</th>");
                  } else {
                    html += "<th class=\"insert-column\">" + OPTION_MAP[lookup_this]["option"] + "</th>";
                  }
                }
              } else if (this.split("-")[0] == "row_total_heading") {
                html += "<th>".concat(measures[this.split("-")[1]].label_short, " ").concat(config.total_header_suffix, "</th>");
              } else {
          ...
          html += '</tr></thead>';
          console.log("html with headings", html); // Build table body

          html += '<tbody>'; // If there is a total row at the bottom of the table - NEED TO TEST THIS BC HAVEN'T SEEN THIS YET
          ...
          if (queryResponse.has_totals == true) {
            ...
            if (data[0][Object.keys(data[0])[0]].value.split(" ")[0] != "All") {
              ...
              var insertArray = Object.keys(queryResponse.totals_data);
              console.log("insertRow pre", insertRow);
              console.log("insertArray", insertArray);
              insertArray.forEach(function (entry) {
                var fillemup = {};

                for (var _i = 0, _Object$entries = Object.entries(queryResponse.totals_data[entry]); _i < _Object$entries.length; _i++) {
          ...
          var _loop = function _loop(len, _i2) {
            html += "<tr><td class=\"table-index\">" + (_i2 + 1) + "</td>";
            $.each(headings_dim, function () {
              // Add the chart columns
              if (this.startsWith("chart_column")) {
                var chart_id = "".concat(this, "-").concat(_i2); // console.log("chart_column html", this, chart_id)

                html += "<td class=\"no-padding\"><div class=".concat(this, " id=") + chart_id + '></div></td>'; // add the last period columns
              } else if (this.startsWith("last_period")) {
                var number_label = this.split("-")[1];
                var last_period_id = "".concat(this, "-").concat(_i2);
                var last_period_class;
                var direction_icon;
                var last_period_data;
                var date_keys = Object.keys(data[_i2][measures[OPTION_MAP[this]["where"]].name]);
                date_keys = date_keys.filter(function (entry) {
                  return entry[0] != "$";
                }); // if the actual cell/data point value isn't null

                if (data[_i2][measures[OPTION_MAP[this]["where"]].name][date_keys[date_keys.length - 1]]["value"] != null) {
                  // if there is a rendered value for the specific data point
                  if (data[_i2][measures[OPTION_MAP[this]["where"]].name][date_keys[date_keys.length - 1]]["rendered"]) {
                    last_period_data = data[_i2][measures[OPTION_MAP[this]["where"]].name][date_keys[date_keys.length - 1]]["rendered"];
                  } else {
                    // if there is no rendered, then use the actual value
                    last_period_data = data[_i2][measures[OPTION_MAP[this]["where"]].name][date_keys[date_keys.length - 1]]["value"];
                  }
                } else {
                ...
                if (data[_i2][measures[OPTION_MAP[this]["where"]].name][date_keys[date_keys.length - 1]]["value"] != null && data[_i2][measures[OPTION_MAP[this]["where"]].name][date_keys[date_keys.length - 2]]["value"] != null) {
                  if (data[_i2][measures[OPTION_MAP[this]["where"]].name][date_keys[date_keys.length - 1]]["value"] < data[_i2][measures[OPTION_MAP[this]["where"]].name][date_keys[date_keys.length - 2]]["value"]) {
                    // if the last period was a decrease
                    last_period_class = "value-down";
                    direction_icon = "<div id=\"chart_triangle-".concat(number_label, "-").concat(_i2, "\"></div>");
                  } else if (data[_i2][measures[OPTION_MAP[this]["where"]].name][date_keys[date_keys.length - 1]]["value"] > data[_i2][measures[OPTION_MAP[this]["where"]].name][date_keys[date_keys.length - 2]]["value"]) {
                    ...
                    direction_icon = "<div style=\"display:none\" id=\"chart_triangle-".concat(number_label, "-").concat(_i2, "\"></div>");
                  }
                } else {
                ...
                html += "<td class=\"triangle-flex\"><div class=".concat(last_period_class, " id=").concat(last_period_id, "><div class=\"change-figure\">").concat(last_period_data, "</div>").concat(direction_icon, "</div></td>"); // if the column is a row total column
              } else if (this.split("-")[0] == "row_total_heading") {
                // if there is a value, use rendered if it exists, otherwise use value
                if (data[_i2][measures[this.split("-")[1]]["name"]]["$$$_row_total_$$$"]["value"]) {
                  // if there is a total value
                  if (data[_i2][measures[this.split("-")[1]]["name"]]["$$$_row_total_$$$"]["rendered"]) {
                    if (where_conditional.includes(parseInt(this.split('-')[1]))) {
                      html += "<td class=\"value-even\" style=\"background:".concat(condition_setups[parseInt(this.split('-')[1])]["scale"](data[_i2][measures[this.split("-")[1]]["name"]]["$$$_row_total_$$$"]["value"]), ";\">").concat(data[_i2][measures[this.split("-")[1]]["name"]]["$$$_row_total_$$$"]["rendered"], "</td>");
                    } else {
                      html += "<td class=\"value-even\">".concat(data[_i2][measures[this.split("-")[1]]["name"]]["$$$_row_total_$$$"]["rendered"], "</td>");
                    }
                  } else {
                    if (where_conditional.includes(parseInt(this.split('-')[1]))) {
                      html += "<td class=\"value-even\" style=\"background:".concat(condition_setups[parseInt(this.split('-')[1])]["scale"](data[_i2][measures[this.split("-")[1]]["name"]]["$$$_row_total_$$$"]["value"]), ";\">").concat(data[_i2][measures[this.split("-")[1]]["name"]]["$$$_row_total_$$$"]["value"], "</td>");
                    } else {
                      html += "<td class=\"value-even\">".concat(data[_i2][measures[this.split("-")[1]]["name"]]["$$$_row_total_$$$"]["value"], "</td>");
                    }
                  }
                }
              } else {
                if (data[_i2][this]["rendered"]) {
                  html += "<td>".concat(data[_i2][this]["rendered"], "</td>");
                } else if (data[_i2][this]["value"] == null) {
                  html += "<td><span class='null-val'>\u2205</span></td>";
                } else {
                  html += "<td>".concat(data[_i2][this]["value"], "</td>");
                }
              }
            });
            html += '</td>';
          ...
          for (var _i2 = 0, len = data.length; _i2 < len; _i2++) {
            _loop(len, _i2);
          }
        ...
        var drawOnTable = function drawOnTable(data_insert, element) {
          ...
          if (where_values.length < 1) {
            ...
            if (config.where_chart.length > 0) {
              where_chart.forEach(function (d, i) {
                var node_get_width = src_select("#chart_column-".concat(i, "-0"));
                var new_node_width = node_get_width.select(function () {
                  return this.parentNode;
                });
                var width_first = new_node_width.node().getBoundingClientRect().width;
                widths.push(width_first);
              });
            }

            console.log("Widths - fixed & variable", widths); // Standardize things if y scale is fixed

            if (config.y_axis_scale_fixed == "true") {
              // gather widths and heights for each chart column based on first row
              if (config.where_chart.length > 0) {
                where_chart.forEach(function (d, i) {
                  var node_get_height = src_select("#chart_column-".concat(i, "-0"));
                  var new_node_height = node_get_height.select(function () {
                    return this.parentNode;
                  });
                  var height_first = new_node_height.node().getBoundingClientRect().height;
                  heights.push(height_first);
                });
              }

              console.log("FIXED y scale heights", heights); // determine mins and maxs for each chart column

              var _loop2 = function _loop2(j, len) {
                where_values.forEach(function (d, i) {
                  var dataset = [];
                  var dataset_keys = Object.keys(data[j][measures[where_values[i]].name]);
                  dataset_keys.forEach(function (entry) {
                    if (entry[0] != "$") {
                      dataset.push({
                        data: entry,
                        value: data[j][measures[where_values[i]].name][entry]["value"]
                      });
                    }
                  });
                  var new_col_min = src_min([col_mins[i], src_min(dataset, function (ent) {
                    return ent.value;
                  })]);
                  col_mins[i] = new_col_min;
                  var new_col_max = src_max([col_maxs[i], src_max(dataset, function (ent) {
                    return ent.value;
                  })]);
            ...
            var _loop3 = function _loop3(_i3, _len) {
              // CYCLE THROUGH EACH CHART COLUMN (j)
              where_values.forEach(function (d, j) {
                var dataset = [];
                var dataset_keys = Object.keys(data[_i3][measures[where_values[j]].name]);
                dataset_keys.forEach(function (entry) {
                  if (entry[0] != "$") {
                    dataset.push({
                      date: entry,
                      value: data[_i3][measures[where_values[j]].name][entry]["value"],
                      rendered: data[_i3][measures[where_values[j]].name][entry]["rendered"]
                    });
                  }
                });

                if (config.y_axis_scale_fixed == "false") {
                  var node_get_height = src_select("#chart_column-".concat(j, "-").concat(_i3));
                  var new_node_height = node_get_height.select(function () {
                    return this.parentNode;
                  });
                  var height_first = new_node_height.node().getBoundingClientRect().height;
                  heights[j] = height_first;
                  col_mins[j] = src_min(dataset, function (ent) {
                    return ent.value;
                  });
                  col_maxs[j] = src_max(dataset, function (ent) {
                    return ent.value;
                  }); // console.log("VARIABLE y-scale heights", heights)
                  // console.log("VARIABLE y-scale mins and maxs", col_mins, col_maxs)
                }

                if (OPTION_MAP["chart_column-".concat(j)]["ylower"] == "" && OPTION_MAP["chart_column-".concat(j)]["yupper"] == "") {
                  y.domain([col_mins[j], col_maxs[j]]);
                } else if (OPTION_MAP["chart_column-".concat(j)]["ylower"] != "" && OPTION_MAP["chart_column-".concat(j)]["yupper"] != "") {
                  y.domain([OPTION_MAP["chart_column-".concat(j)]["ylower"], OPTION_MAP["chart_column-".concat(j)]["yupper"]]);
                } else if (OPTION_MAP["chart_column-".concat(j)]["ylower"] != "") {
                  y.domain([OPTION_MAP["chart_column-".concat(j)]["ylower"], col_maxs[j]]);
                } else {
                  y.domain([col_mins[j], OPTION_MAP["chart_column-".concat(j)]["yupper"]]);
                }
                ...
                var chart_id_grabber = "#chart_column-".concat(j, "-").concat(_i3);
                var triangle_id_grabber = "#chart_triangle-".concat(j, "-").concat(_i3);
                var svg = src_select(chart_id_grabber).append('svg').html('').attr("width", widths[j]).attr('height', heights[j]).classed("svg-".concat(j), true);
                var group = svg.append('g').attr("transform", "translate(".concat(margin.left, ", ").concat(margin.top, ")")).attr("width", widths[j] - margin.left - margin.right).attr("height", heights[j] - margin.top - margin.bottom).classed("group", true);

                if (config.text_directionality == "true") {
                  var icons = src_select(triangle_id_grabber).append("svg").attr("width", 30).attr("height", 30).append("g").attr("transform", "translate(15, 18)");
                  var tri = icons.append("path").attr("d", src_symbol().type(triangle).size(30));

                  if (dataset[dataset.length - 1].value < dataset[dataset.length - 2].value) {
                    tri.attr("transform", "rotate(180)").style("fill", "#D76106");
                  } else if (dataset[dataset.length - 1].value > dataset[dataset.length - 2].value) {
                    tri.style("fill", "#0072b5");
                  }
                } // if (config.text_directionality == "true") {
                // } else {
                //     if (dataset[dataset.length - 1].value < dataset[dataset.length - 2].value) {
                ...
                if (OPTION_MAP["chart_column-".concat(j)]["chart"] == "line") {
                  group.append("path").data([dataset]).attr("class", "line").attr("d", line).attr("stroke", function (e) {
                    if (config.directionality == "true") {
                      var allValues = dataset.map(function (a, ai) {
                        return a.value;
                      });
                      var firstNonNull = allValues.filter(function (arg) {
                        return arg != null;
                      })[0]; // if first value of chart is more than last value, consider it decrease/orange
                      ...
                      if (dataset[dataset.length - 1].value != null && firstNonNull > dataset[dataset.length - 1].value) {
                        return "#d76106"; // } else if (dataset[dataset.length - 1].value != null && dataset[0].value < dataset[dataset.length - 1].value) {
                      ...
                      return ["#27566b", "#007b82"][_i3 % 2]; // alternate bw blue and teal
                    }
                  });
                  group.selectAll(".dot").data(dataset.filter(function (d, i) {
                    return d.value !== null;
                  })).enter().append("circle").attr("class", "dot").attr("cx", line.x()).attr("cy", line.y()).attr("r", 1).attr("fill", function (d) {
                    if (config.directionality == "true") {
                      var allValues = dataset.map(function (a, ai) {
                        return a.value;
                      });
                      ...
                      return ["#27566b", "#007b82"][_i3 % 2];
                    }
                  });
                } else if (OPTION_MAP["chart_column-".concat(j)]["chart"] == "area") {
                  group.append("path").data([dataset]).attr("class", "area").attr("d", area).attr("stroke", function (d) {
                    if (config.directionality == "true") {
                      var allValues = dataset.map(function (a, ai) {
                        return a.value;
                      });
                ...
                } else if (OPTION_MAP["chart_column-".concat(j)]["chart"] == "bar") {
                  var xBar = band().domain(dataset.map(function (d, i) {
                    return d.date;
                  })).range([0, widths[j] - margin.left - margin.right]).padding(0.05);
                  group.selectAll(".bar").data(dataset).enter().append("rect").attr("class", "bar").attr("x", function (d) {
                    return xBar(d.date);
                  }).attr("y", function (d) {
                    return y(Math.max(0, d.value));
                  }).attr("width", xBar.bandwidth()).attr("height", function (d) {
                ...
                var arr_text = [];
                var val_extent = src_extent(dataset, function (d, i) {
                  return d.value;
                });
                var minValValues = {};
                var maxValValues = {};
                ...
                if (isNaN(+val_extent[0])) {
                  ...
                  arr_text.push({
                    "value": val_extent[0],
                    "date": minValValues['date'],
                    "rendered": minValValues['rendered']
                  });
                  arr_text.push({
                    "value": val_extent[1],
                    "date": maxValValues['date'],
                    "rendered": maxValValues['rendered']
                  });

                  if (config.display_values == "true") {
                    var filter = group.append("defs").append("filter").attr("id", "blur").append("feGaussianBlur").attr("stdDeviation", .8);
                    group.selectAll(".min-max-text-background").data(arr_text).enter().append("text").attr("class", "min-max-text-background").attr("x", function (d, i) {
                    ...
                    }).attr("font-size", 8).attr("font-family", "Roboto").attr("font-weight", 700).attr("fill", "white").attr("text-anchor", function (d, i) {
                    ...
                    }).style("opacity", .4).attr("filter", "url(#blur)");
                    group.selectAll(".min-max-text").data(arr_text).enter().append("text").attr("class", "min-max-text").attr("x", function (d, i) {
                      if (pivots[0].time_interval.name == "month" && x(parseTime(d.date)) > 20) {
                        return x(parseTime(d.date));
                      } else {
                        return x(parseTime(d.date));
                    ...
                    }).attr("font-size", 8).attr("font-weight", 700).attr("text-anchor", function (d, i) {
                      if (x(parseTime(d.date)) < 12) {
                        return "start"; // "beginning"
                      } else if (x(parseTime(d.date)) < 100) {
                        return "middle";
            ...
            for (var _i3 = 0, _len = data.length; _i3 < _len; _i3++) {
              _loop3(_i3, _len);
            }
          }
        }; // -----------------------------------------------------------------------------------------
        ...
        var parseTimeDay = timeParse("%Y-%m-%d"); // for day or "other" (week)

        var parseTimeMonth = timeParse("%Y-%m"); // for month

        var parseTimeYear = timeParse("%Y"); // for year

        var parseTimeSecond = timeParse("%Y-%m-%d %H:%M:%S"); // for exact time

        var parseTime; // table calculations are included in measure-like and can be found by is_table_calculation = true
        ...
        if (config.where_chart.length < 1) {
          ...
          config.where_chart.split(',').forEach(function (d, i) {
            if (d) {
        ...
        if (config.where_values.length < 1) {
          ...
          config.where_values.split(',').forEach(function (d, i) {
            if (d) {
        ...
        if (config.where_measure_totals.length < 1) {
          ...
          config.where_measure_totals.split(',').forEach(function (d, i) {
            if (d) {
        ...
        var where_conditional = [];

        if (config.where_conditional_formatting.length < 1) {
          where_conditional = [];
        } else {
          config.where_conditional_formatting.split(',').forEach(function (d, i) {
            if (d) {
        ...
        var totals_diverging = [];

        if (config.totals_diverging.length < 1) {
          totals_diverging = [];
        } else {
          config.totals_diverging.split(',').forEach(function (d, i) {
            if (d) {
        ...
        if (config.totals_negative.length < 1) {
          ...
          config.totals_negative.split(',').forEach(function (d, i) {
            if (d) {
        ...
        var condition_setups = {};

        if (where_conditional != []) {
          where_conditional.forEach(function (d, i) {
            if (where_totals.includes(d)) {
              // has to be a column chose to show total for
              var col_extent = src_extent(data, function (e, j) {
                return e[measures[d].name]["$$$_row_total_$$$"].value;
              });

              if (totals_diverging.includes(d) && col_extent[0] < 0) {
                var condition_scale = diverging().domain([col_extent[0], 0, col_extent[1]]).interpolator(src_value("rgba(223,128,55,1.0)", "rgba(25,128,188,1.0)"));
                condition_setups[d] = {
                  'extent': col_extent,
                  'scale': condition_scale
                };
              } else if (totals_negative.includes(d)) {
                var _condition_scale = linear_linear().range(["rgba(241, 204, 86, 1.0)", "rgba(253, 249, 238, 0.8)"]).domain(col_extent);
                ...
                var _condition_scale2 = linear_linear().range(["rgba(245, 249, 242, 0.8)", "rgba(140, 187, 97, 0.8)"]).domain(col_extent);

                condition_setups[d] = {
                  'extent': col_extent,
                  'scale': _condition_scale2
                };
              }
            }
          });
        ...
        console.log("condition_setups", condition_setups); // gather headings

        var row_totals = [];
        var column_headers = []; // Get header names for the dimensions
        ...
        if (where_chart.length > 0) {
          for (var i = where_chart.length - 1; i >= 0; i--) {
            // reverse order
            if (config.add_last_period_stats == "true") {
              column_headers.splice(where_chart[i], 0, "last_period_column-".concat(i));
            }

            column_headers.splice(where_chart[i], 0, "chart_column-".concat(i));
          }
        }
        ...
        var x = src_time().range([0, 116]);
        var y = linear_linear().range([46, 0]); // Define charting functions

        var line = src_line().defined(function (d) {
          return d.value != null;
        }).x(function (d) {
          return x(parseTime(d.date));
        }).y(function (d) {
        ...
        var area = d3_shape_src_area().x(function (d) {
          return x(parseTime(d.date));
        }).y0(function (d) {
          return y(0);
        }).y1(function (d) {
...
var template_keys = Object.keys(sparklinesNew_object.options);
console.log("object.options", sparklinesNew_object.options);
console.log("Object", Object.keys(sparklinesNew_object.options));
console.log("keys", template_keys);
template_keys.forEach(function (entry, i) {
  var array_name = sparklinesNew_object.options[entry].label;
  ...
  if (["radio", "select"].includes(sparklinesNew_object.options[entry].display)) {
    var array_values = sparklinesNew_object.options[entry].values;
    array_values.forEach(function (ent) {
      var str;

      if (ent[Object.keys(ent)[0]] == sparklinesNew_object.options[entry]["default"]) {
  ...
  } else if (sparklinesNew_object.options[entry].display == "number") {
    var str;
    str = "<input type='number' internal_cat='" + template_keys[i] + "' internal_value='" + sparklinesNew_object.options[entry]["default"] + "' id='" + template_keys[i] + "' min='0' name='" + sparklinesNew_object.options[entry]["label"] + "' value='" + sparklinesNew_object.options[entry]["default"] + "'></input><label class='form-label' for='" + sparklinesNew_object.options[entry]["label"] + "'>" + sparklinesNew_object.options[entry]["label"] + "</label>";
    form.append(str);
  } else if (sparklinesNew_object.options[entry].display == "text") {
    var _str;

    _str = "<input type='text' internal_cat='" + template_keys[i] + "' internal_value='" + sparklinesNew_object.options[entry]["default"] + "' id='" + template_keys[i] + "' name='" + sparklinesNew_object.options[entry]["label"] + "' value='" + sparklinesNew_object.options[entry]["default"] + "'></input><label class='form-label' for='" + sparklinesNew_object.options[entry]["label"] + "'>" + sparklinesNew_object.options[entry]["label"] + "</label>";
    form.append(_str);
  } else if (sparklinesNew_object.options[entry].type == "boolean") {
    var _array_values = ["true", "false"];

    _array_values.forEach(function (ent) {
      var str;

      if (ent == sparklinesNew_object.options[entry]["default"]) {
  ...
  } else if (sparklinesNew_object.options[entry].type == "array") {
    var _array_values2 = sparklinesNew_object.options[entry]["default"]; // console.log("array_values", array_values)
...
json("http://localhost:3001/dataSparklineIH").then(function (data) {
  //dataMCSmap2 dataSankey2 dataSparklineWorks
  var todays_options = {};
  ...
  sparklinesNew_object.updateAsync(data.data, src_select("#vis")._groups[0][0], todays_options, data.queryResponse, details, template_done, this_environment); // On change to options, loop through selections and then redraw chart

  jquery('input:radio').on("click", function () {
    jquery('input:radio:checked').each(function () {
      todays_options[this.attributes.internal_cat.value] = this.attributes.internal_value.value;
    });
    sparklinesNew_object.updateAsync(data.data, src_select("#vis")._groups[0][0], todays_options, data.queryResponse, details, template_done, this_environment);
  });
  ...
  var mouseDownHandler = function mouseDownHandler(e) {
    ...
    h = parseInt(styles.height, 10); // Attach the listeners to `document`
