...
var weekCumulative_object = {
  ...
  updateAsync: function updateAsync(data, element, config, queryResponse, details, done) {
    ...
    try {
      ...
      var firstWeek = data_ready.filter(function (element) {
        return element.day.getTime() === new Date(new Date(new Date().getFullYear(), new Date().getMonth(), 1).setHours(0, 0, 0, 0)).getTime();
      })[0]['week'];
