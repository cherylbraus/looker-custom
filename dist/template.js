...
function Sankey() {
  ...
  function computeNodeValues(graph) {
    graph.nodes.forEach(function (node) {
      node.value = Math.max(src_sum(node.sourceLinks, sankey_value), src_sum(node.targetLinks, sankey_value));
    });
  ...
  function computeNodeBreadths(graph) {
    ...
    function initializeNodeBreadth() {
      var ky = src_min(columns, function (nodes) {
        return (y1 - y0 - (nodes.length - 1) * py) / src_sum(nodes, sankey_value);
      });
...
// CONCATENATED MODULE: ./node_modules/d3-sankey/src/index.js
  ...
  updateAsync: function updateAsync(data, element, config, queryResponse, details, done) {
    ...
    try {
      ...
      var sankey = Sankey().nodeId(function (d) {
        return d.id;
      }).nodeWidth(20).nodePadding(200).size([width, height]).nodeAlign(align_center);
      ...
      }).attr("font-size", "10px").attr("text-anchor", "end").attr("transform", null).text(function (d) {
        return d.name;
      }).filter(function (d) {
        return d.x0 < width / 2;
      }).attr("x", function (d) {
...
var template_keys = Object.keys(sankey_object.options);
console.log("object.options", sankey_object.options);
console.log("Object", Object.keys(sankey_object.options));
console.log("keys", template_keys);
template_keys.forEach(function (entry, i) {
  var array_name = sankey_object.options[entry].label;
  ...
  if (["radio", "select"].includes(sankey_object.options[entry].display)) {
    var array_values = sankey_object.options[entry].values;
    array_values.forEach(function (ent) {
      var str;

      if (ent[Object.keys(ent)[0]] == sankey_object.options[entry]["default"]) {
  ...
  } else if (sankey_object.options[entry].display == "number") {
    var str;
    str = "<input type='number' internal_cat='" + template_keys[i] + "' internal_value='" + sankey_object.options[entry]["default"] + "' id='" + template_keys[i] + "' min='0' name='" + sankey_object.options[entry]["label"] + "' value='" + sankey_object.options[entry]["default"] + "'></input><label class='form-label' for='" + sankey_object.options[entry]["label"] + "'>" + sankey_object.options[entry]["label"] + "</label>";
    form.append(str);
  } else if (sankey_object.options[entry].display == "text") {
    var _str;

    _str = "<input type='text' internal_cat='" + template_keys[i] + "' internal_value='" + sankey_object.options[entry]["default"] + "' id='" + template_keys[i] + "' name='" + sankey_object.options[entry]["label"] + "' value='" + sankey_object.options[entry]["default"] + "'></input><label class='form-label' for='" + sankey_object.options[entry]["label"] + "'>" + sankey_object.options[entry]["label"] + "</label>";
    form.append(_str);
  } else if (sankey_object.options[entry].type == "boolean") {
    var _array_values = ["true", "false"];

    _array_values.forEach(function (ent) {
      var str;

      if (ent == sankey_object.options[entry]["default"]) {
  ...
  } else if (sankey_object.options[entry].type == "array") {
    var _array_values2 = sankey_object.options[entry]["default"]; // console.log("array_values", array_values)
...
json("http://localhost:3001/dataSankey").then(function (data) {
  ...
  sankey_object.updateAsync(data.data, src_select("#vis")._groups[0][0], todays_options, data.queryResponse, details, template_done, this_environment); // On change to options, loop through selections and then redraw chart

  jquery('input:radio').on("click", function () {
    jquery('input:radio:checked').each(function () {
      todays_options[this.attributes.internal_cat.value] = this.attributes.internal_value.value;
    });
    sankey_object.updateAsync(data.data, src_select("#vis")._groups[0][0], todays_options, data.queryResponse, details, template_done, this_environment);
  });
