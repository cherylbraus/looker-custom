(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		exports["sankeyNew"] = factory();
	else
		root["sankeyNew"] = factory();
})(window, function() {
...
// CONCATENATED MODULE: ./src/integration/sankey/sankey.js
...
looker.plugins.visualizations.add({
  ...
  updateAsync: function updateAsync(data, element, config, queryResponse, details, done) {
    ...
    try {
      ...
      var redraw = function redraw(sank_map_all) {
        ...
        function walkLinks() {
          var _ref = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {},
              _ref$obj = _ref.obj,
              obj = _ref$obj === void 0 ? srclinks : _ref$obj,
              _ref$action = _ref.action,
              action = _ref$action === void 0 ? "over" : _ref$action;

          if (obj.length > 0) {
            obj.forEach(function (a, i) {
              if (action === "over") {
                src_select(".link.ind".concat(String(a["index"]))).attr("stroke-opacity", 0.8).attr("stroke", "#a8bbc3");
              } else {
                src_select(".link.ind".concat(String(a["index"]))).attr("stroke-opacity", 0.4).attr("stroke", "#afafaf");
              }

              walkLinks({
                obj: a.target.sourceLinks,
                action: action
              });
            });
          }
        ...
        function nodeHighlightLinks(d) {
          ...
          src_select(".node.id".concat(String(d.id))).attr("fill", "#27566b").attr("opacity", 1.0);
        }
        ...
        function nodeUnHighlightLinks(d) {
          ...
          src_select(".node.id".concat(String(d.id))).attr("fill", "#27566b").attr("opacity", 0.7);
        } // -----------------------------------------------------------------------------------
        ...
        }).attr("font-size", "11px").attr("text-anchor", "end").attr("dominant-baseline", "middle").attr("transform", null).text(function (d) {
        ...
        var labelValues = group.append('g').attr("class", "node-labels").selectAll("text").data(graph.nodes).enter().append("text").attr("class", "node-label value").attr("x", function (d) {
          return d.x0 - 2;
        }).attr("y", function (d) {
          return d.y0 + (d.y1 - d.y0) / 2;
        }).attr("dy", 1.1 + "em").attr("font-size", "10px").attr("text-anchor", "end").attr("dominant-baseline", "middle").attr("transform", null).text(function (d) {
          return "(".concat(defaultLocale_format(NUMFORMAT)(d.value), ")");
        }).filter(function (d) {
          return d.x0 < width / 2;
        }).attr("x", function (d) {
          return d.x1 + 2;
      ...
      var dropdownTitle = jquery(".dropdown-title");
      dropdownTitle.text("".concat(dimensions[0].label, ": "));
      var listDropdown = jquery("#vis-options");
      listDropdown.empty();
      data.forEach(function (entry, i) {
        if (i == 0) {
          listDropdown.append(jquery("<option></option>").attr("value", entry[dimensions[0].name].value).text(entry[dimensions[0].name].value).attr("selected", "selected"));
        } else {
