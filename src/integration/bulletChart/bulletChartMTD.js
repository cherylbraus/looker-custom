...
looker.plugins.visualizations.add({
    ...
    options: {
        ...
        bar_display: {
          type: 'string',
          label: 'Color bars based on above/below a goal',
          display: "radio",
          values: [
            {"Yes": "yes"},
            {"No": "no"}
...
updateAsync: function(data, element, config, queryResponse, details, done, environment = "prod") {
        ...
        if (config.bar_display === "yes") {
            options.color.hidden = true
        } else {
            options.color.hidden = false
        }
...
try {
    ...
    const percPlanLabels = rightLabels
        ...
        .enter()
            ...
            .text(d => d3.format(".0%")(d["monthly-actual"]/d[`monthly-${config.comparison.split("-")[1]}`]))
