...
export const object = {
    ...
    updateAsync: function(data, element, config, queryResponse, details, done, environment = "prod") {
        ...
        try {
            ...
            const monthGroups = group.append("g")
                .attr("class", "month-groups")
                .selectAll("g")
                .data(data_ready)
                .enter()
                .append("g")
                    .attr("class", d => `singlemonth ${d3.timeFormat("%b'%-y")(monthAccessor(d))}`)
            ...
            const comps = monthGroups
                .append("rect")
                    .attr("x", d => xScale(monthAccessor(d)) - 0.04*(xScale.bandwidth()))
                    .attr("y", d => yScale(forecastAccessor(d)))
                    .attr("width", xScale.bandwidth() + 0.08*(xScale.bandwidth()))
                    .attr("height", 4)
                    .attr("fill", "lightgrey")
                    .attr("stroke", "black")
                    .attr("stroke-width", 1)
            ...
            const hoverWeekBars = monthGroups
                .append("rect")
